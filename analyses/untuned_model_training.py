...
if 'pred_glm' in df_test.columns:
    df_test['pred_glm_t'] = y_pred_GLM  # replace existing
else:
    df_test['pred_glm_t'] = y_pred_GLM  # create new column

if 'pred_lgbm' in df_test.columns:
    df_test['pred_lgbm_t'] = y_pred_LGBM
else:
    df_test['pred_lgbm_t'] = y_pred_LGBM
...
project_root = Path(__file__).resolve().parents[1]
output_path = project_root / "d100project" / "evaluation" / "df_test_with_tuned_predictions.parquet"
